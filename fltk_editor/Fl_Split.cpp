//
// "$Id: Fl_Split.cxx,v 1.5.2.5 2001/01/22 15:13:40 easysw Exp $"
//
...
void Fl_Split :: resize(int X,int Y,int W,int H) {
  ...
  // If resize exceeds minimums, assume child is not visible
  // so ignore the  resize  call and only allows change position. 
  // So let split and children sizes as frozen 
  // This allows minimizing when Fl_Split is a child area of 
  // MDI child window 
  if ( W <= 0 || H <= 0 )
  {
...
static void set_cursor(Fl_Split*t, Fl_Cursor c) {
...
int Fl_Split::handle(int event)
{
	  static int sdrag = 0;
	  static int sdx = 0, sdy = 0;
...
#define GRABAREA 8
	  ...
	  case FL_RELEASE: {
		  ...
		  // Some parent seems to be unable to redraw the widget so 
		  // we pass this to the window.
		  window()->redraw();
		  do_callback();
		  return 1;}
...
//
// End of "$Id: Fl_Split.cxx,v 1.5.2.5 2001/01/22 15:13:40 easysw Exp $".
//
