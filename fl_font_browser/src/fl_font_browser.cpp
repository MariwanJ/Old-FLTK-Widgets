//////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////
//  fl_font_browser.cpp      v 0.0.3                              2020-08-24 
//
//         for the Fast Light Tool Kit (FLTK) 1.3.x .
//
//    by Mariwan Jalal
//
...
Fl_Font_Browser *ft=(Fl_Font_Browser*)frmWid;
  ...
  for (int i=1; i<= ft->lst_Font->size();i++)// Search for the font Full name 
   {
	  std::string st1 = ft->lst_Font->text(i);
	  std::string st2 = ft->txt_InputFont->value();
	  ToUpperCase(st2);
	  ToUpperCase(st1);
...
Fl_Font_Browser *ft=(Fl_Font_Browser*)frmWid;
  ...
  for (int i=1; i<= ft->lst_Style->size();i++)// Search for the font Full name 
   {
	  std::string st1 = ft->lst_Style->text(i);
	  std::string st2 = ft->txt_InputStyle->value();
	  ToUpperCase(st2);
	  ToUpperCase(st1);
	  if (st1.compare(st2) == 0) {
...
int Fl_Font_Browser::GetFontNr(const char *fontNametoNr)
{
	...
	{
		st1 = lst_Style->text(lst_Style->value());
		ToLowerCase(st1);
    }
       else{
     st1= lst_Font->text(lst_Font->value());
	}
   int i ;     
   for (i=0;;i++)
   {
    const char *name = Fl::get_font_name((Fl_Font)i,&t); 
    if (strcmp(name,st1.c_str())==0)
    {
       return i;   
    }
   }    
